...
Готово! После успешного запуска сервер будет доступен по адресу **`127.0.0.1:8080`** на компьютере-хосте и по адресу **`10.0.2.2:8080`** в эмуляторе на том же компьютере. Для других устройств (в той же сети) доступно по `IP_КОМПЬЮТЕРА:8080`. 
>Внимание: иногда компьютер может не пускать соединения с других устройств из-за настроек брандмауэра. В таком случае:
> <kbd>Win</kbd> + <kbd>R</kbd>, ввести `wf.msc`, выбрать `Для входящих подключений` (вверху слева), `Создать правило` (вверху справа), select `Для порта` → `TCP` → Определенные порты: `8080` → **Разрешить подключение**
